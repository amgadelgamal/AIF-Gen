...
from benchmarks.COPR.trlx.data.default_configs import (
    default_ilql_config,
    default_ppo_config,
    default_sft_config,
)
from benchmarks.COPR.trlx.utils import set_seed
from benchmarks.COPR.trlx.utils.loading import get_pipeline, get_trainer
...
def train(  # noqa: C901
    ...
    if config.method.name.lower() in ['spinconfig']:
        assert eval_prompts is not None
        assert prompts is not None
        assert labels is not None
        max_label_length = config.method.gen_kwargs['max_new_tokens']
        ...
        if reward_fn:  # Online training against a reward function (e.g. PPO)
        ...
        # Offline training from the collected samples (e.g. SFT, ILQL)
            ...
            if rewards is not None:
                ...
                if config.method.name.lower() in [
                    'dpoconfig',
                    'rdpoconfig',
                    'rdposftconfig',
                    'coprconfig',
                    ...
                    trainer.make_experience(
                        samples=samples,
                        input_length=max_prompt_length,
                        output_length=config.method.gen_kwargs['max_new_tokens'],
                    )
            ...
            raise ValueError(
                'Either `samples` or `reward_fn` should be given for training'
            )
        ...
        if config.method.name.lower() in [
            ...
            eval_pipeline = get_pipeline('PromptPipeline')(
                eval_prompts,
                max_prompt_length,
                trainer.tokenizer,
                add_special_tokens=config.model.model_arch_type == 'seq2seq',
