...
from benchmarks.COPR.trlx.trainer import BaseRLTrainer, register_trainer
from benchmarks.COPR.trlx.utils import (
    filter_non_scalars,
...
from benchmarks.COPR.trlx.utils.logging import get_logger, tqdm
...
class AccelerateRLTrainer(BaseRLTrainer):
    ...
    def __init__(self, config, **kwargs):  # noqa: C901
        ...
        if config.train.minibatch_size:
            assert (
                config.train.batch_size % config.train.minibatch_size == 0
        ...
        if (
            'llama_zh_' in config.tokenizer.tokenizer_path.lower()
            or 'chatmind' in config.tokenizer.tokenizer_path.lower()
            or 'colossal-llama-2' in config.tokenizer.tokenizer_path.lower()
        ):
            ...
            logger.info(f'Use LlamaTokenizer: {config.tokenizer.tokenizer_path}')
        else:
        ...
        for i_sweep, gen_sweep_value in enumerate(gen_sweep_values):
            ...
            for i_prompt, prompts in enumerate(self.eval_dataloader):
                metadata = {
                    ...
                    if k
                    not in ['input_ids', 'attention_mask', 'label_ids', 'label_len']
                }
                if self.generate_sweep_kwarg:
                    ...
                    samples = self.generate_eval(
                        prompts['input_ids'], prompts['attention_mask']
                    )
            ...
            if self.accelerator.is_main_process:
                ...
                if self.metric_fn:
                    ...
                    mean_metrics = {
                        f'metrics/{k}{sweep_suffix}': torch.as_tensor(xs, dtype=float)
                        .mean(-1)
                        .item()
    ...
    def _accumulate(self):
        ...
        self.mb_count += 1
        assert (
            self.mb_count // self.num_mb <= self.config.train.total_steps
