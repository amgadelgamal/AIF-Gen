...
from benchmarks.COPR.trlx.data.ilql_types import ILQLBatch, ILQLSeq2SeqBatch
from benchmarks.COPR.trlx.models.modeling_ilql import (
    AutoModelForCausalLMWithILQLHeads,
    AutoModelForSeq2SeqLMWithILQLHeads,
    ILQLConfig,
)
from benchmarks.COPR.trlx.pipeline.offline_pipeline import (
    ILQLRolloutStorage,
...
from benchmarks.COPR.trlx.utils import to_device
...
def make_experience(samples, rewards, tokenizer=None, max_length=2048, verbose=True):  # noqa: C901
    ...
    for sample in samples:
        ...
        for dm in sample:
            if dm.is_output:
                actions_ixs.append(
                    torch.arange(length - 1, length + len(dm.tokens) - 1)
                )
...
class AccelerateILQLTrainer(AccelerateRLTrainer):
    ...
    def make_experience_seq2seq(self, samples, rewards, max_length=2048):
        ...
        if self.tokenizer:
            samples = [
                tokenize_dialogue(s, self.tokenizer, max_length) for s in samples
            ]
        ...
        sample_lengths = np.array(list(map(len, all_input_ids))) + np.array(
            list(map(len, all_output_ids))
        )
        ...
        returns = (returns - returns.mean()) / (
            returns.std() + torch.finfo(returns.dtype).eps
        )
        rewards = [torch.zeros(len(x)) for x in all_actions_ixs]
        for rs, ret in zip(rewards, returns):
            rs[-1] = ret
    ...
    def make_experience(self, samples, rewards, max_length=2048):
        ...
        self.store = make_experience(
            samples, rewards, self.tokenizer, max_length=max_length, verbose=True
        )
