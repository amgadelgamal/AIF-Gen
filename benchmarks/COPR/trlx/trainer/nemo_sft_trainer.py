...
from benchmarks.COPR.trlx.models.modeling_nemo_sft import SFTGPT
from benchmarks.COPR.trlx.trainer import BaseRLTrainer, register_trainer
from benchmarks.COPR.trlx.trainer.accelerate_sft_trainer import SFTConfig
from benchmarks.COPR.trlx.trainer.nemo_ilql_trainer import (
    ShuffledCyclicSequence,
    megatron_trainer,
)
...
class NeMoSFTTrainer(BaseRLTrainer):
    ...
    def learn(self):
        ...
        def collate_fn(elems: List[transformers.BatchEncoding]):
            ...
            input_ids, loss_mask = pad_batch_and_build_loss_mask(
                context_tokens, self.max_length
            )
        ...
        def eval_collate(elems):
            context_tokens = [
                add_special_token_ids(
                    e['input_ids'],
                    add_bos=self.model.cfg.data.get('add_bos', False),
                    add_eos=False,
                )
                for e in elems
            ]
