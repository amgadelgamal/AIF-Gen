...
from benchmarks.COPR.trlx.models.modeling_ppo import (
    AdaptiveKLController,
    AutoModelForCausalLMWithHydraValueHead,
    AutoModelForSeq2SeqLMWithHydraValueHead,
    FixedKLController,
)
from benchmarks.COPR.trlx.pipeline.cppo_pipeline import CPPORolloutStorage
from benchmarks.COPR.trlx.pipeline.offline_pipeline import PromptPipeline
...
from benchmarks.COPR.trlx.utils import Clock, infinite_dataloader
...
@register_trainer
class AccelerateCPPOTrainer(AccelerateRLTrainer):
    ...
    def __init__(self, config: TRLConfig, **kwargs):
        ...
        # CPPO adds
        self.running_logP = RunningMoments()
        self.running_R = RunningMoments()
    ...
    def loss(self, batch: PPORLBatch):
        ...
        loss, stats = self.config.method.loss(
            ...
            extra=self.store.extra,  # CPPO adds 传一些均值和方差
        )
    ...
    # CPPO adds
    def push_to_store(self, data, extra=None):
        self.store.push(data)
        if extra is not None:
            self.store.extra = extra
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        while len(ppo_rl_elements) < num_rollouts:
            ...
            # 计算 log_logits 均值和方差
            logP = torch.tensor([item.mean() for item in all_logprobs]).to(device)
            logP_mean, logP_std = self.running_logP.update(logP)
            stats['old_logP/mean'] = logP_mean
            stats['old_logP/std'] = logP_std

            # 计算 rewards[-1] 均值和方差
            old_rewards = torch.tensor(
                [item.rewards[-1] for item in ppo_rl_elements[-n_samples:]]
            ).to(device)
            rewards_mean, rewards_std = self.running_R.update(old_rewards)
            stats['old_rewards/mean'] = rewards_mean
            stats['old_rewards/std'] = rewards_std
        ...
        self.push_to_store(ppo_rl_elements, stats)
