...
def create_report(
    target_metric, column_names, entity_name, project_name, group_name, best_config
):
    report = wb.Report(
        ...
        wb.PanelGrid(
            panels=[
                wb.ParallelCoordinatesPlot(
                    columns=[wb.PCColumn(f'c::{column}') for column in column_names]
                    + [wb.PCColumn(target_metric)],
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Number of GPUs (workers) to use per trial.',
    )
    parser.add_argument(
        '--num_cpus',
        type=int,
        default=4,
        help='Number of CPUs to use per GPU (worker).',
    )
    parser.add_argument(
        '-y', '--assume_yes', action='store_true', help="Don't ask for confirmation"
    )
    parser.add_argument(
        '--server_address',
        type=str,
