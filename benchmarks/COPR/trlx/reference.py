...
parser.add_argument(
    '--against', type=str, default='CarperAI/trlx:main', help='Reference git branch'
)
parser.add_argument(
    '--public',
    action='store_true',
    help='Use CarperAI entity to store/pull from w&b runs',
)
...
out = os.popen(
    f'./scripts/benchmark.sh --origin {pr_origin} --branch {pr_branch} --only_hash'
)
pr_hash, pr_git_hash = [x[:-1] for x in out.readlines()]

out = os.popen(
    f'./scripts/benchmark.sh --origin {ref_origin} --branch {ref_branch} --only_hash'
)
...
if runs:
    print(
        f'On {ref_branch} @{ref_git_hash} these runs were already made: \n{chr(10).join(run.name for run in runs)}'
    )
else:
    print(f'Making runs on {ref_branch} @{ref_git_hash}')
    subprocess.run(
        f'./scripts/benchmark.sh --origin {ref_origin} --branch {ref_branch} {public}'.split()
    )

runs = api.runs(project_name, filters={'tags': {'$in': [pr_hash]}})
if runs:
    print(
        f'On {pr_branch} @{pr_git_hash} these runs were already made: \n{chr(10).join(run.name for run in runs)}'
    )
else:
    print(f'Making runs on {pr_branch} @{pr_git_hash}')
    subprocess.run(
        f'./scripts/benchmark.sh --origin {pr_origin} --branch {pr_branch} {public}'.split()
    )
...
for name in experiment_names:
    ...
    metrics = set(
        sum(
            [
                [
                    metric
                    for metric in run.history().columns
    ...
    blocks.extend(
        [
            ...
            wb.PanelGrid(
                panels=[
                    panel for panel in metrics_panels if panel.title in major_metrics
                ],
