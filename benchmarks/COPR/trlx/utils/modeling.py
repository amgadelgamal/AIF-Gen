...
def freeze_emb_causal_layers(model: nn.Module):
    """Freezes the bottom transformer block layers of the specified model."""

    def hf_get_emb_blocks(model: nn.Module) -> Tuple[nn.Module]:
        ...
        """
        emb_layers_attrs = (
            'model.embedding',  # embedding for gpt2
            'model.embed_tokens',  # embedding for llama
            'model.decoder.embed_tokens',  # embedding for opt
            'model.decoder.embed_positions',  # embed_positions for opt
        )
        all_blocks = []
        for attr in emb_layers_attrs:
            if rhasattr(model, attr):
                all_blocks.append(rgetattr(model, attr))
                print(f'modeling.py line 192 print: finded: {attr}')
        assert len(all_blocks) > 0, 'No embedding parameters finded!'
        return all_blocks

    # 冻结emb，降低内存
    emd_layers = hf_get_emb_blocks(model)
    for block in list(emd_layers):
        block.requires_grad_(False)

    # 获取模型中需要计算梯度的参数
    params_to_update = filter(lambda p: p.requires_grad, model.parameters())
    params_to_freeze = filter(lambda p: not p.requires_grad, model.parameters())

    # 计算参数总量
    update_params = sum(p.numel() for p in params_to_update)
    freeze_params = sum(p.numel() for p in params_to_freeze)

    print(f'训练参数量: {update_params}, 冻结参数量: {freeze_params}')
