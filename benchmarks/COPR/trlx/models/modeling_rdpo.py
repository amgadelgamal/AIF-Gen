...
class RDPOConfig(MethodConfig):
    ...
    # coef_stb: float = 0.0
    margin_rm: float = 0.0
    margin_dpo: float = 0.0
    freeze_emb: bool = False
    log_lambda: float = 0.0
    lambda_lr: float = 1e-3
    constraint_threshold: float = 0.0
    # moving_average_lopP: float = 0.0
    # moving_average_lopP_alpha: float = 0.9
    rdpo_type: str = '[score_reward]_[ref_rdpo]'
    ...
    def loss(
        ...
        # l2_loss = (logprobs - old_logprobs).square().mean()
        l2_loss = (
            (
        ...
        # loss = pg_loss + self.vf_coef * vf_loss + 0.1*l2_loss
...
class AutoModelForCausalLMWithValueHead(PreTrainedModelWrapper):
    ...
    def __init__(
        ...
        # var1 = torch.var(self.v_head[0].weight.data).item()
        # avg1 = torch.mean(self.v_head[0].weight.data).item()
        # print(f'##### Init var1:{var1} avg1:{avg1}')
    ...
    #         wrapped_model_kwargs, from_pretrained_kwargs = self._split_kwargs(kwargs)
    #     else:
    ...
    #         base_model = self._auto_model_parent_class.from_pretrained(
    #             pretrained_model_name_or_path, *model_args, trust_remote_code=True,revision=revision, **from_pretrained_kwargs
    #         )
    #
    #         ### 新增 load head
    #         bin_file = "pytorch_model.bin"
    #         index_file = f"{pretrained_model_name_or_path}/pytorch_model.bin.index.json"
    #         if os.path.exists(index_file):
    #             with open(index_file) as f:
    #                 index = json.load(f)
    #             weight_map = index['weight_map']
    #             if "v_head.2.weight" in weight_map:
    #                 bin_file = weight_map["v_head.2.weight"]
    #         print(bin_file)
    #         bin_data = torch.load(f"{pretrained_model_name_or_path}/{bin_file}", map_location=torch.device('cpu'))
    #         self.v_head.weight.data = bin_data["v_head.2.weight"]
    #         self.v_head.bias.data = bin_data["v_head.2.bias"]
    #         print(f"self.v_head in re-loaded from {pretrained_model_name_or_path}/{bin_file}")
    #
    #         ############################
    #
    ...
    #         )
    #
    #     model = self(base_model, **wrapped_model_kwargs)
    #
    #     if isinstance(pretrained_model_name_or_path, str):
