...
from benchmarks.COPR.trlx.data.ilql_types import ILQLBatch
    ...
    def sync_target_q_heads(self):
        if os.environ.get('ACCELERATE_DEEPSPEED_ZERO_STAGE', '0') == '3':
            with deepspeed.zero.GatheredParameters(
                list(self.parameters()), modifier_rank=0
            ):
...
class AutoModelForCausalLMWithILQLHeads(PreTrainedModelWrapper):
    ...
    def generate(
        ...
        pad_token_id = (
            ...
            else self.base_model.config.pad_token_id
        )
        eos_token_id = (
            eos_token_id
            if eos_token_id is not None
            else self.base_model.config.eos_token_id
        )
        ...
        for _ in range(max_new_tokens):
            ...
            if os.environ.get(
                'ACCELERATE_DEEPSPEED_ZERO_STAGE', '0'
            ) != '3' and torch.all(finished):
                break
