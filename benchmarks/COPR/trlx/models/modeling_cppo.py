...
"""
version v0
原始coff learn & reg 1.0 - > 1.5/0.5

version v1 2023 03 03
原始coff learn & reg 1.0 -> 2.0/0.5, 2.0/0.2
阈值设置为 avg + 0.85*std

version v2 2023 0704
abl_type: repeat
### 为了解决重复问题设置的特殊CPPO

version v3 2023 0706
vf_loss 使用 coff-learn
...
class CPPOConfig(MethodConfig):
    ...
    reg_coef: float  # cppo adds
    abl_type: str  # cppo adds
    threhold: float  # cppo adds
    ub: float  # cppo adds
    lb: float  # cppo adds
    gen_kwargs: dict
    ...
    def detect_track(self, stats, old_logprobs, old_rewards, mask):
        """old_logprobs : [B, responce_size]
        old_rewards : [B, responce_size]
        """
                ...
                if cond11[i]:
                    ...
                    if cond21[i]:  # low retention & high learning
                        ...
                        coff_learn[i] = max(
                            1
                            + self.threhold
                            + (old_logprobs[i, lenth[i]] - logP_mean) / rewards_std,
                            0.5,
                        )
                        coff_reg[i] = max(
                            1
    ...
    def loss(
        ...
        old_rewards: TensorType['batch_size', 'response_size'],
        extra: Dict,
    ):
        ...
        """
        coff_learn, coff_reg = self.detect_track(extra, old_logprobs, old_rewards, mask)
        ...
        vf_clipfrac = (
            torch.sum((vf_loss2 > vf_loss1).float() * mask_alpha) / mask_alpha.sum()
        )
        ...
        # 加入权重 coff_learn
        pg_loss = (
            torch.sum(torch.max(pg_loss1, pg_loss2) * mask_alpha) / mask_alpha.sum()
        )
        pg_clipfrac = (
            torch.sum((pg_loss2 > pg_loss1).float() * mask_alpha) / mask_alpha.sum()
        )
        ...
        if 'norm' in self.abl_type:
            l2_loss = (
                (
                    logprobs / logprobs.norm(dim=-1, keepdim=True)
                    - old_logprobs / old_logprobs.norm(dim=-1, keepdim=True)
                ).square()
                * mask_beta
            ).sum() / mask_beta.sum()
        else:
            l2_loss = (
                (logprobs - old_logprobs).square() * mask_beta
            ).sum() / mask_beta.sum()

        loss = pg_loss + self.vf_coef * vf_loss + self.reg_coef * l2_loss
        ...
        stats = dict(
            ...
            cppo=dict(
                coff_learn_ratio=(
                    coff_learn.not_equal(1.0).float().sum() / coff_learn.shape[0]
                ).item(),
                coff_learn_mean=coff_learn.float().mean().item(),
                coff_learn_std=coff_learn.float().std().item(),
                coff_reg_ratio=(
                    coff_reg.not_equal(1.0).float().sum() / coff_reg.shape[0]
                ).item(),
                coff_reg_mean=coff_reg.float().mean().item(),
                coff_reg_std=coff_reg.float().std().item(),
            ),
        )
...
class T5Branch(ModelBranch):
    ...
    def forward(  # noqa: max-complexity
        ...
        if self.is_decoder and encoder_hidden_states is not None:
            ...
            if encoder_attention_mask is None:
                encoder_attention_mask = torch.ones(
                    encoder_hidden_shape, device=hidden_states.device
                )
