...
from benchmarks.COPR.trlx.data.ilql_types import (
    ILQLBatch,
    ILQLElement,
    ILQLSeq2SeqBatch,
    ILQLSeq2SeqElement,
...
def tokenize_dialogue(  # noqa: C901
    ...
    if tokenizer.truncation_side == 'left':
        tokenized = [
            DialogMessage(is_output=m.is_output, tokens=m.tokens[::-1])
            for m in tokenized[::-1]
        ]
...
class DialogStore(BaseRolloutStore):
    def __init__(
        self, dialogs: List[List[DialogMessage]], tokenizer: PreTrainedTokenizer
    ):
        super().__init__()
        self.tokenizer = tokenizer
        attention_masks = [
            torch.ones(sum(len(m.tokens) for m in d), dtype=torch.bool) for d in dialogs
        ]
        input_ids = [
            torch.tensor([t for m in d for t in m.tokens], dtype=torch.long)
            for d in dialogs
        ]
        ...
        self.history = [
            dict(input_ids=i, attention_mask=a, labels=l)
    ...
    def create_loader(self, batch_size: int, shuffle=False) -> DataLoader:
        ...
        def collate_fn(elems: Iterable[dict]):
            ...
            labels = hf_collate_fn([{'input_ids': e['labels']} for e in elems])[
                'input_ids'
            ]
            batch['labels'] = labels
            return batch
    ...
    def create_loader(self, batch_size: int, shuffle=False) -> DataLoader:
        def collate_fn(xs):
            out = self.tokenizer.pad(
                [{'input_ids': x['input_ids']} for x in xs], return_tensors='pt'
            )
...
def ilql_seq2seq_collate_fn(elems: Iterable[ILQLElement]):
    return ILQLSeq2SeqBatch(
        ...
        pad_sequence(
            [x.decoder_input_ids for x in elems], batch_first=True, padding_value=0
        ),
        pad_sequence([x.rewards for x in elems], batch_first=True, padding_value=0.0),
        pad_sequence([x.states_ixs for x in elems], batch_first=True, padding_value=0),
        pad_sequence([x.actions_ixs for x in elems], batch_first=True, padding_value=0),
